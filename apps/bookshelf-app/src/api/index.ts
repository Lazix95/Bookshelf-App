...
export const sendRequest = <T>(request: Request): Promise<AxiosResponse<T>> => {
  ...
  return axiosInstance
    ...
    .catch((err) => {
      if (requestToSend.errorToastMessage && err.response.data.msg)
        Vue.$toast.error(err.response.data.msg);
      if (
        requestToSend.errorToastMessage &&
        err.response.data.validationMessages
      ) {
        Object.values(err.response.data.validationMessages).forEach(
          (message) => {
