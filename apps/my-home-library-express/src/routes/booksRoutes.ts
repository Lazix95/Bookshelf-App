...
import { deleteBook, getBooks, getBookToReq, getOneBook, postBook, putBook } from '../controllers/booksController';
...
router.get('/books/:bookID', isAuth, isObjectId, runValidator, getBookToReq, getOneBook);

router.post('/books', isAuth, multer, required('name'), required('author'), required('publisher'), runValidator, uploadFile, postBook);
router.put('/books/:bookID', isAuth, multer, isObjectId, required('name'), required('author'), required('publisher'), runValidator, uploadFile, getBookToReq, putBook);
